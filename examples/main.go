...
import (
	...
	vips "github.com/szytko/vipsgo/vips"
)
...
func testFile() {
	...
	vimg, err := vips.LoadImage(inputImage)
	if err != nil {
	...
	err = vimg.Resize(&vips.ImageResizeOptions{
		MaintainAspect: true,
		Width:          800,
		Height:         600,
	})
	...
	err = vimg.Crop(&vips.ImageCropOptions{X: 50, Y: 50, Width: 400, Height: 300})
	if err != nil {
	...
	err = vimg.Rotate(&vips.ImageRotateOptions{Angle: 15.0})
	if err != nil {
	...
	finalBytes, err := vimg.EncodeToJPEG(&vips.ImageEncodeJPEGOptions{Quality: 85, Interlace: false})
	if err != nil {
	...
	vimgFromBytes, err := vips.LoadImageFromBytes(finalBytes)
	if err != nil {
...
func testBytes() {
	...
	vimg, err := vips.LoadImageFromBytes(imageBytes)
	if err != nil {
		fmt.Printf("   ✗ Error loading image from bytes: %v\n", err)
		return
	}
